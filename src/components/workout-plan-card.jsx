...
const Work_Out_Plan = () => {
  return (
    <div className="min-h-screen bg-black py-10  max-w-3xl md:max-w-4xl xl:max-w-4xl mx-auto">
      <h2 className=" text-center md:text-4xl text-2xl xl:text-4xl md:leading-9 xl:leading-9  font-bold text-[#eb0000] mb-4 md:px-0 lg:px-0 px-6 uppercase">
        Select Your Workout Plan
      </h2>
      <p className="text-[#dcdcdc] md:text-xl xl:text-xl leading-8 text-center second md:px-32 xl:px-32 mb-4">
      ...
      <div className="max-w-7xl mx-auto grid gap-4 md:grid-cols-4 sm:grid-cols-2 grid-cols-1 px-4">
        {levels_list.map((level, index) => (
          <div key={index} className={`  text-white   shadow-lg`}>
            <img
              src={level.image}
              alt={level.title}
